...
> OneMapToRuleThemAll**; plus **FarmGrid**, **FirstPersonMode**, and **FavoriteItems** (optional). Still **removed on 1.0** and NOT to be
...
(mark stacks as favorites with Alt-click) are optional — install whichever you want, skip the rest;
each is a personal preference and either way you can join. The two marked ❌ are **removed** on 1.0
(see the notice at the top) — do not install them until Ethan confirms they have a real 1.0 build.
...
2. Copy the `.dll` files from `BepInEx/plugins/` in this pack into your game's
   `…/Valheim/BepInEx/plugins/` folder. (If you don't want FarmGrid, FirstPersonMode, or
   FavoriteItems, leave out `FarmGrid.dll` / `FirstPersonMode.dll` / `FavoriteItems.dll` —
   they're optional; the rest are required.)
...
- Make sure you have **all five required mods (ModSentry, Drop That, Jotunn, BetterCarts,
  ...
  gets you kicked. (FarmGrid, FirstPersonMode, and FavoriteItems
