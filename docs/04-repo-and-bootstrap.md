...
(push `valheim/` config → run `stage-mods.sh` → restart the container) — but that, and the
full deploy model, the BepInEx `config`-symlink gotcha, and the ModSentry/Jotunn plugin
rules, now live in the **valheim-server** repo (`/srv/dev/repos/valheim-server`): read its
`valheim/README.md` before touching the Valheim mod set. This host repo keeps only the
generic `guests/create-vm.sh` mechanism the Valheim VM wrapper calls.
